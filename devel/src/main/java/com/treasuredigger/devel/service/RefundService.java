...
public class RefundService {
    ...
    public void refundRequest(String accessToken, String merchantUid, String reason) throws IOException {
        ...
        json.addProperty("merchant_uid", merchantUid); // merchant_uid -> merchantUid
        json.addProperty("reason", reason);

        // 출력 스트림으로 해당 conn에 요청
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(conn.getOutputStream()));
        bw.write(json.toString());  // JSON 객체를 문자열로 변환하여 전송
        bw.flush();
        bw.close();
        ...
        log.error("Iamport API 응답 오류: {}", response.toString());  // 오류 응답 로그 추가
        br.close();
        conn.disconnect();

        log.info("결제 취소 완료 : 주문 번호 {}", merchantUid); // merchant_uid -> merchantUid
    }
